...
var listCmd = &cobra.Command{
	Use:   "list",
	Short: "List all authentications for feature experimentation",
	Long:  `List all authentications for feature experimentation`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		for _, fileName := range existingAuths {
			if fileName != "" {
				...
				credPath, err := config.CredentialPath(utils.FEATURE_EXPERIMENTATION, fileName)
				if err != nil {
					log.Fatalf("error occurred: %s", err)
				}
